...
export const Logo = ({ size = "md", variant = "default", className }: LogoProps) => {
  ...
  return (
    <div className={cn("flex items-baseline gap-1.5 select-none", className)}>
      <span className={cn("font-mono font-bold leading-none", isSidebar ? "text-sidebar-primary" : "text-primary", monoSizes[size])}>
        ⟨P⟩
      </span>
      <span className={cn("font-bold tracking-tight translate-y-[1px] leading-none", textSizes[size])}>
